  ...
  return (
    <div className="section-full reveal bg-main reveal" style={{ minHeight: '100vh', paddingTop: '40px' }}>
      <div className="section-inner">
        ...
        <div className="agents-grid" style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '32px' }}>
          {agents.map(agent => (
            <div key={agent.id} className="agent-card-landscape">
              ...
              <div className="acl-body">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
                  <div>
                    <div style={{ fontSize: 'clamp(1.2rem, 3vw, 1.5rem)', fontWeight: '800', color: 'var(--ink)' }}>{agent.name}</div>
                    <div style={{ fontSize: 'clamp(0.9rem, 2vw, 1rem)', color: 'var(--gold)', fontWeight: '700', marginTop: '4px' }}>{agent.designation}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: 'clamp(1rem, 2vw, 1.2rem)', fontWeight: '800', color: 'var(--green)' }}>★ {agent.rating}</div>
                    <div style={{ fontSize: '0.85rem', color: 'var(--muted)', fontWeight: '700' }}>{agent.experience} Exp</div>
                  </div>
                </div>
                
                <p style={{ fontSize: 'clamp(0.9rem, 1.5vw, 1rem)', color: 'var(--muted)', marginBottom: '20px', lineHeight: '1.6' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '30px' }}>
                  {['Global Network', 'RERA Certified', 'Portfolio Mgmt', 'Market Analysis'].map((tag, i) => (
                    <span key={i} style={{ fontSize: '10px', background: 'var(--gold4)', color: 'var(--ink)', padding: '6px 12px', border: '1px solid var(--gold3)', borderRadius: '6px', fontWeight: '700', textTransform: 'uppercase' }}>{tag}</span>
                  ))}
                </div>
